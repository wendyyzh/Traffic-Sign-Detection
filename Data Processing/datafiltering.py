...
if annotations:
    ...
    # Filter classes for data augmentation (between 500 and 1000 instances)
    augmentation_class_counts = {label: count for label, count in class_counts.items() if 500 < count < 1000}
    
    # Filter classes with more than 1000 instances
    filtered_class_counts = {label: count for label, count in class_counts.items() if count >= 1000}
    ...
    plt.title('Number of Instances in Each Class (Classes with >= 1000 Instances)')
    ...
    # Save augmentation data to a new JSON file
